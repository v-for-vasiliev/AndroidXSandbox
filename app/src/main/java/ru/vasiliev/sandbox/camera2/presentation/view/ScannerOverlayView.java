...
public class ScannerOverlayView extends LinearLayout {
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        canvas.drawBitmap(scannerViewBackgroundBitmap,
                          0,
                          0,
                          null);
    }
    ...
    protected Bitmap createScannerViewBackground() {
        Bitmap backgroundBitmap = Bitmap.createBitmap(getWidth(),
                                                      getHeight(),
        ...
        drawBackground(canvas,
                       BACKGROUND_COLOR);
        drawScannerRectangle(canvas,
                             SCAN_RECT_WIDTH,
                             SCAN_RECT_HEIGHT,
                             SCAN_RECT_CORNER_WIDTH,
                             SCAN_CORNERS_COLOR);
    ...
    private void drawBackground(Canvas canvas, @ColorRes int color) {
        RectF outerRectangle = new RectF(0,
                                         0,
                                         getWidth(),
                                         getHeight());
        ...
        paint.setColor(ContextCompat.getColor(getContext(),
                                              color));
        //paint.setAlpha(99);
        canvas.drawRect(outerRectangle,
                        paint);
    }
    ...
    private void drawScannerRectangle(Canvas canvas, int scannerRectWidth, int scannerRectHeight,
        ...
        RectF scanRect = new RectF(centerX - (scannerRectWidth / 2.0f),
                                   ...
                                   centerY + (scannerRectHeight / 2.0f));
        canvas.drawRect(scanRect,
                        paint);

        drawScannerCornersPath(canvas,
                               scanRect,
                               scannerCornersWidth,
                               color);
    }
    ...
    private void drawScannerCornersPath(Canvas canvas, RectF scanRect, int cornerWidth, @ColorRes int color) {
        ...
        path.moveTo(scanRect.left,
                    scanRect.top + cornerWidth);
        path.lineTo(scanRect.left,
                    scanRect.top);
        path.lineTo(scanRect.left + cornerWidth,
                    scanRect.top);

        path.moveTo(scanRect.right - cornerWidth,
                    scanRect.top);
        path.lineTo(scanRect.right,
                    scanRect.top);
        path.lineTo(scanRect.right,
                    scanRect.top + cornerWidth);
