...
public class ScannerOverlayView extends LinearLayout {
    ...
    private void drawScannerRectangle(Canvas canvas,
                                      int scannerRectWidth,
                                      int scannerRectHeight,
                                      int scannerCornersWidth,
                                      @ColorRes int color) {
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        ...
        RectF scanRect = new RectF(centerX - (scannerRectWidth / 2.0f), centerY - (scannerRectHeight / 2.0f),
                                   centerX + (scannerRectWidth / 2.0f), centerY + (scannerRectHeight / 2.0f));
        canvas.drawRect(scanRect, paint);

        drawScannerCornersPath(canvas, scanRect, scannerCornersWidth, color);
    }

    private void drawScannerCornersPath(Canvas canvas,
                                        RectF scanRect,
                                        int cornerWidth,
                                        @ColorRes int color) {
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
