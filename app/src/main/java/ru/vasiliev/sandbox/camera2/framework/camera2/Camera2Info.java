...
class Camera2Info {
    ...
    boolean isLegacyHardware() {
        try {
            return requireNonNull(characteristics.get(
    ...
    boolean isAutoFocusSupported() {
        ...
        boolean afModeSupported = contains(CameraMetadata.CONTROL_AF_MODE_AUTO, supportedAfModes) || contains(
                CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE, supportedAfModes);
    ...
    void printCameraCharacteristics() {
        ...
        dbg("Hardware level: %s", Camera2Utils.getHardwareLevelString(
