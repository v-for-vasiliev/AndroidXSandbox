...
import ru.vasiliev.sandbox.legacycamera.camera2.exception.CameraInvalidModeException;
import ru.vasiliev.sandbox.legacycamera.camera2.exception.CameraNotReadyToCaptureException;
import ru.vasiliev.sandbox.camera.device.scanner.BarcodeScanner;
import ru.vasiliev.sandbox.camera.presentation.view.AutoFitTextureView;
import ru.vasiliev.sandbox.camera.presentation.view.FocusSurfaceView;
...
import static ru.vasiliev.sandbox.legacycamera.camera2.CameraDbg.dbg;
...
public class Camera2Api implements View.OnTouchListener {
    ...
    private final CameraDevice.StateCallback getCameraOpenStateCallback() {
        return new CameraDevice.StateCallback() {
            ...
            public void onOpened(@NonNull CameraDevice cameraDevice) {
                cameraInfo.printCameraCharacteristics();
                if (cameraInfo.isDeviceCameraSupported(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL_LEGACY)) {
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                cameraInfo = new CameraInfo(cameraCharacteristics);
    ...
    private void setupPreviewRequestParams(CaptureRequest.Builder requestBuilder) {
        ...
        if (cameraInfo.isAutoFocusSupported()) {
        ...
        if (cameraInfo.isAutoExposureSupported()) {
            requestBuilder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_ON);
            aeMode = "CONTROL_AE_MODE_ON";
        }

        if (cameraInfo.isAWBSupported()) {
    ...
    private void setupCaptureRequestParams(CaptureRequest.Builder requestBuilder, int rotation) {
        ...
        if (cameraInfo.isAutoFocusSupported() && !manualFocusLocked) {
            int[] afModes = cameraCharacteristics.get(CameraCharacteristics.CONTROL_AF_AVAILABLE_MODES);
            if (contains(CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE, afModes)) {
                requestBuilder.set(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_CONTINUOUS_PICTURE);
            } else {
    ...
    private void lockFocus(MotionEvent event) {
        ...
        try {
            ...
            if (cameraInfo.isAeMeteringAreaSupported()) {
            ...
            if (cameraInfo.isAfMeteringAreaSupported()) {
